    ...
    <header className="sticky top-0 z-50 bg-zinc-950/90 backdrop-blur-md border-b border-zinc-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden text-zinc-300 hover:text-white p-2">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-zinc-900 border-b border-zinc-800 absolute w-full">
          <div className="px-4 pt-2 pb-6 space-y-2">
            {siteConfig.nav.map((item) => (
              <Link key={item.href} href={item.href} onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-4 text-base font-bold text-zinc-300 hover:text-white hover:bg-zinc-800 uppercase tracking-wider">
                {item.label}
              </Link>
            ))}
            <a href={`https://wa.me/${siteConfig.company.whatsapp}`} className="block w-full text-center mt-4 bg-red-600 text-white px-3 py-4 font-bold uppercase tracking-wider rounded-sm">
              WhatsApp Destek
            </a>
          </div>
        </div>
